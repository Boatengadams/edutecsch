...
export const StudentView: React.FC<{ isSidebarExpanded: boolean; setIsSidebarExpanded: (v: boolean) => void; }> = ({ isSidebarExpanded, setIsSidebarExpanded }) => {
  ...
  const navItems = useMemo(() => {
    ...
    return (savedOrder.map(key => itemMap.get(key)).filter(Boolean) as any[]) || rawItems;
              ...
              return (
                  <div className="space-y-10 animate-fade-in-up">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                           <Card onClick={() => setActiveTab('live_lesson')} className="lg:col-span-2 !bg-blue-600/5 border-blue-500/20 relative overflow-hidden group min-h-[250px] flex flex-col justify-between rounded-[3rem] cursor-pointer hover:bg-blue-600/10 transition-all hover:scale-[1.01]">
                                ...
                                <Button onClick={(e) => { e.stopPropagation(); setActiveTab('live_lesson'); }} disabled={!liveLesson} className="w-full sm:w-fit py-4 px-12 rounded-[2rem] font-black uppercase tracking-[0.3em] shadow-2xl shadow-blue-900/30">Connect Link 🚀</Button>
                           </Card>
                           <div className="flex flex-col gap-6">
                               <Card className="flex flex-col justify-between p-8 cursor-pointer hover:bg-slate-800 hover:scale-[1.02] transition-all rounded-[2.5rem] group" onClick={() => setActiveTab('assignments')}>
                                    ...
                                    <p className="text-[9px] font-black uppercase text-blue-500 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">Open Task List →</p>
                               </Card>
                               <Card className="flex flex-col justify-between p-8 rounded-[2.5rem] cursor-pointer hover:bg-slate-800 hover:scale-[1.02] transition-all group" onClick={() => setActiveTab('profile')}>
                                    ...
                                    <p className="text-[9px] font-black uppercase text-blue-500 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">View Rank →</p>
                               </Card>
                           </div>
                      </div>
                  </div>
