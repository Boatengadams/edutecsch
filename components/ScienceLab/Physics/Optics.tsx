...
const Optics: React.FC<OpticsProps> = ({ onUpdateChar }) => {
    ...
    const handleMouseDown = (e: React.MouseEvent | React.TouchEvent, id: string) => {
        ...
        dragRef.current = { isDragging: true, id, mouseX, mouseY, offsetX: mouseX - comp.x, offsetY: mouseY - comp.y };
    ...
    return (
        <div className="h-full flex flex-col-reverse md:flex-row bg-transparent relative" onClick={() => { setSelectedId(null); setSidebarOpen(false); }}>
            <button onClick={(e) => { e.stopPropagation(); setSidebarOpen(!isSidebarOpen); }} className="absolute top-4 left-4 z-50 p-3 bg-slate-800 rounded-full text-white shadow-lg border border-slate-600 transition-all">
                {isSidebarOpen ? '✖️' : '🔍'}
            </button>
             <div className={`absolute top-0 left-0 h-full w-64 bg-[#0B0F19] border-r border-slate-800 z-40 shadow-2xl transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} onClick={(e) => e.stopPropagation()}>
                 <div className="p-4 pt-16 flex flex-col h-full">
                    <div className="mb-4"><input type="text" placeholder="Search optics..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full p-2 bg-slate-800 rounded-lg text-xs text-white border border-slate-700 outline-none" /></div>
                    ...
                    <div className="grid grid-cols-2 gap-2 flex-grow content-start overflow-y-auto custom-scrollbar">
                        {filteredTools.map(tool => (
                            <button key={tool.id} onClick={() => addComponent(tool)} className="flex flex-col items-center p-2 bg-slate-800/50 rounded-xl border border-slate-700 hover:border-purple-500 transition-all">
                                <span className="text-2xl mb-1">{tool.icon}</span><span className="text-[9px] text-slate-300 font-bold text-center">{tool.name}</span>
                            </button>
                        ))}
                    </div>
                    <button onClick={() => setComponents([])} className="mt-auto w-full py-3 bg-red-500/10 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold mt-6">Clear Bench</button>
                </div>
            </div>
            <div ref={benchRef} className="flex-grow relative bg-transparent overflow-auto custom-scrollbar shadow-inner cursor-crosshair touch-none">
                ...
                {components.map(comp => {
                    ...
                    return (
                        <div key={comp.id} className={`absolute cursor-grab active:cursor-grabbing ${isSelected ? 'z-30' : 'z-20'}`} style={{ left: comp.x, top: comp.y }} onMouseDown={(e) => handleMouseDown(e, comp.id)} onTouchStart={(e) => handleMouseDown(e, comp.id)}>
                             {isSelected && (
                                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-4 bg-slate-800 p-2 rounded-xl border border-slate-600 shadow-xl z-50 flex flex-col items-center gap-2 w-32 animate-fade-in-up" onMouseDown={e => e.stopPropagation()}>
                                     <div className="flex w-full justify-between items-center"><span className="text-[10px] font-bold text-white truncate">{comp.name}</span><button onClick={() => deleteComponent(comp.id)} className="text-red-400 hover:text-red-300"><span className="text-xs">🗑️</span></button></div>
                                     <div className="w-full"><input type="range" min="-180" max="180" value={comp.rotation} onChange={(e) => updateRotation(comp.id, Number(e.target.value))} className="w-full h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-purple-500" /></div>
                                </div>
                            )}
                            <div className={`${isSelected ? 'ring-2 ring-purple-500 ring-offset-2 ring-offset-slate-900 rounded-lg' : ''}`}><ComponentVisual comp={comp} /></div>
                        </div>
                    );
                })}
            </div>
