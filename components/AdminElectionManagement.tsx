                ...
                <div className="flex bg-slate-950 p-1 rounded-2xl border border-white/5 w-full md:w-auto overflow-x-auto scrollbar-hide">
                    {['management', 'timeline', 'vetting', 'results'].map(mode => (
                        <button 
                            ...
                            className={`flex-1 md:flex-none whitespace-nowrap px-4 sm:px-6 py-2 rounded-xl text-[8px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === mode ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-xl' : 'text-slate-600 hover:text-white'}`}
                        >
                            {mode.replace('_', ' ')}
                        </button>
                    ))}
